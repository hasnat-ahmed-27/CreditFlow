"""
Engine/session bootstrap for the Scheduler service.

Same lazy pattern as the other services: the engine is created in `init_db()`
(FastAPI lifespan — and at the top of every Celery task, since worker/beat
processes never run the lifespan), not at import time, so the Docker
build-time smoke test (`python -c "import main"`) doesn't need a live
Postgres.

Schema strategy: `make_engine(schema="scheduler")` isolates this service's
...
def init_db() -> None:
    ...
    if config.DATABASE_URL.startswith("sqlite"):
        ...
        engine = make_engine(schema="scheduler")

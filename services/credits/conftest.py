...
_TMP = tempfile.mkdtemp(prefix="creditflow_credits_test_")
_DB_FILE = os.path.join(_TMP, f"credits_{uuid.uuid4().hex}.db")
...
os.environ["CREDITS_CONSUMER_ENABLED"] = "0"   # no broker in tests
...
def published_events(monkeypatch):
    ...
    [(routing_key, payload)]. Covers both the routes and the consumer —
    each calls events.publish."""
...
def client():
    ...
    # Each test starts from an empty database — processed_events and the
    # marketplace browse endpoint are service-global, so leftover rows from a
    # previous test would poison assertions.

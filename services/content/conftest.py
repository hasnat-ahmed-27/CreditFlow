...
with the throwaway key and the app verifies with its public half), a stub
publisher instead of RabbitMQ, and a temp directory instead of the media
volume. The consumer thread is disabled — tests call consumer.handle_event
directly, exercising the exact function the broker would. No Redis: this
service doesn't use it. This is why the suite runs in CI with no infra
...
_TMP = tempfile.mkdtemp(prefix="creditflow_content_test_")
_DB_FILE = os.path.join(_TMP, f"content_{uuid.uuid4().hex}.db")
...
os.environ["CONTENT_CONSUMER_ENABLED"] = "0"   # no broker in tests
os.environ["CONTENT_MEDIA_DIR"] = _MEDIA       # image uploads land in the temp dir
...
def published_events(monkeypatch):
    ...
    [(routing_key, payload)]. Both routes and the consumer publish through

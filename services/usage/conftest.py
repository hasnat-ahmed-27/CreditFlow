...
instead of Redis, and a stub publisher instead of RabbitMQ. The consumer
thread is disabled — tests call consumer.handle_event directly, exercising
the exact function the broker would. This is why the suite runs in CI with
no infra containers and no secrets.
"""
...
_TMP = tempfile.mkdtemp(prefix="creditflow_usage_test_")
_DB_FILE = os.path.join(_TMP, f"usage_{uuid.uuid4().hex}.db")
...
os.environ["USAGE_CONSUMER_ENABLED"] = "0"   # no broker in tests
os.environ["USAGE_QUOTA_TOKENS"] = "1000"    # small quota -> thresholds at 800/1000
...
def fake_redis(monkeypatch):
    """Fresh in-memory Redis per test (isolates the per-account counters)."""
    monkeypatch.setattr(store, "_client", fakeredis.FakeRedis(decode_responses=True))


@pytest.fixture(autouse=True)
def published_events(monkeypatch):
    """Capture events instead of talking to RabbitMQ; returns
    [(routing_key, payload)]. The consumer is the only publisher here."""
